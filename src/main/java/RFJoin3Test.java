            ...
            try (PreparedStatement statement = connection.prepareStatement(selectQuery)) {
                ...
                try (ResultSet resultSet = statement.executeQuery()) {
                    ...
                    for (YourResultType yourResultType : resultList) {
                        statement1.addBatch("SELECT * FROM Folder WHERE folderCP = '" + yourResultType.folderCP + "';");
                    }
                    int[] result = statement1.executeBatch();

                    for (int i = 0; statement1.getMoreResults(); i++) {
                        resultList.get(i).accountCP = resultList.get(i).accountCP;
                    }
                }
