...
public class RFJoin5Test {
        ...
        try (Connection connection = DriverManager.getConnection(url, username, password)) {
            ...
            try (PreparedStatement statement = connection.prepareStatement(selectQuery)) {
                ...
                try (ResultSet resultSet = statement.executeQuery()) {
                    ...
                    String q = "SELECT * FROM Folder WHERE " + buildInClause(resultList.size());
                    PreparedStatement preparedStatement1 = connection.prepareStatement(q);

                    for (int i = 0; i < resultList.size(); i++) {
                        preparedStatement1.setString(i + 1, resultList.get(i).folderCP);
                    }

                    ResultSet resultSet1 = preparedStatement1.executeQuery();

                    for (int i = 0; resultSet.next(); i++) {
                        resultList.get(i).accountCP = resultSet1.getString(1);
                    }
                }
    ...
    private static String buildInClause(int size) {
        StringBuilder sb = new StringBuilder(" folderCP = ? ");
        for (int i = 1; i < size; i++) {
            sb.append("OR folderCP = ? ");
        }
        sb.append(";");
        return sb.toString();
    }
